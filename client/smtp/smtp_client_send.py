...
def main(argv):
    if len(argv) != 5:
        print(
            "usage: ./<name>.py <OD2 IP Address> <OD1 IP Address> <TimeoutInSeconds> <src_port> Your Len "
            + str(len(argv))
        )
        exit(1)
    ...
    smtp_helper.login_send_mail(
        sender_email, sender_passwd, recv_email, subject_txt, body_txt
    )
    ...
    while cipher_text is None:
        time.sleep(2)
        cipher_text = smtp_helper.login_find_single_mail(
            sender_email, sender_passwd, recv_email, PING_A_ACK_SUB
        )
