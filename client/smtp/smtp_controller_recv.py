...
def send_ack_to_client(email_to_send_to, client_public_key):
    ...
    smtp_helper.send_mail(
        CONTROLLER_EMAIL,
        CONTROLLER_EMAIL_PASSWD,
        email_to_send_to,
        CLIENT_A_ACK_SUB,
        body_txt,
    )
...
def main():
    ...
    payloads_list, email_id_list = smtp_helper.login_recv_all_mail(
        CONTROLLER_EMAIL, CONTROLLER_EMAIL_PASSWD, CLIENT_MAIL, CLIENT_A_SUBJECT
    )
    ...
    for index, iPayload in enumerate(payloads_list):
        ...
        if magic_wrd == MAGIC_WORD:
            ...
            print(
                "Add rule to Controller : " + email_id_list[index] + "  Send self ping "
            )
