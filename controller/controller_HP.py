...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def add_redirection_flows(self, dp, buffer_id, srcport, dstip, timeout, srcip):
        ...
        match1 = parser.OFPMatch(
            eth_type=0x0800, ipv4_src=srcip, ipv4_dst=overt_ip
        )  # , tcp_src = int(srcport) )
    ...
    def add_flow(
        ...
        if redirect_table:
            ...
            msg = parser.OFPFlowMod(
                ...
                match=match1,
                instructions=inst,
            )  # more priority than the controller, but less than the decoy flows
    ...
    def _packet_in_handler(self, ev):
        ...
        if pkt_icmp:
            ...
            if "DecoyRKC" in icmpData:
                print "Got a Decoy Routing PING"
                timeout = (
                    0  # in case client doesn't supply one, we will use OVS default
                )
                timeoutString = icmpData[
                    icmpData.find("DecoyRKC") + 5 : icmpData.find("@")
                ]
                dstip = ip_addr_ntoa(
                    icmpData[icmpData.find("@") + 1 : icmpData.find("#")]
                )
                # dstip="192.168.2.4"
                ...
                self.add_redirection_flows(
                    datapath, msg.buffer_id, srcport, dstip, timeout, srcip
                )
        ...
        if out_port != ofproto.OFPP_FLOOD:
            ...
            if msg.buffer_id != ofproto.OFP_NO_BUFFER:
                ...
                # print "adding flow2"
                self.add_flow(
                    datapath, 2, match, actions, table_id=200, redirect_table=False
                )  # ,  src_ip = ipv.src_ip, dst_ip=ipv.dst_ip)
        if msg.buffer_id == ofproto.OFP_NO_BUFFER:
            data = msg.data
